...
- 设备 Token 90 天过期，Token 不在管理 API 中暴露
- 书籍上传大小限制 200MB
- 书籍正文静态文件访问认证
- EPUB 解析 CSS 路径验证（防路径遍历）
- 公开封面资源直链访问
- 安全 HTTP 头（X-Frame-Options, X-XSS-Protection）
- HTML 清理防护
- 管理员重置阅读时长后，已结束的会话不再计入防沉迷判断
...
- Server 防沉迷 `canStartReading` 已加入连续阅读滚动窗口检查，超限后必须休息才能开始新会话
- Server `resetDailyReading` 重置逻辑已修复，重置后已结束会话不再计入防沉迷判断和每日统计
- Server `closeDanglingReadingSessions` 已补充 `updateDailyStats` 调用，统计数据不再缺失
- Server 登录流程已增加 Session ID 重新生成，防止会话固定攻击
- Server 设备列表 API 不再暴露 device_token，仅返回当前管理员设备
- Server 书籍上传已增加 200MB 大小限制
- Server EPUB 解析 CSS 路径已增加目录边界验证
- Web 设备管理页面已移除无效的"设备Token"列
- Web Dashboard/Stats 页面已修复切换子账号时的竞态条件
- Web API 层已统一使用 `unwrapData` 替代非空断言，错误提示更清晰
- Web 登出时已清空全部 Store 数据，防止下一个用户看到残留信息
- Web 书籍预览已修复 XSS 风险（renderBaseUrl 转义处理）
